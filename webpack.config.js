...
const { CleanWebpackPlugin } = require("clean-webpack-plugin"); // для удаления лишнего из папки build
const MiniCssExtractPlugin = require("mini-css-extract-plugin"); // для того чтоб в build создавался отдельный css фаил, а не встроенный в html
...
const isLocalServer = process.env.NODE_LOC === "locServ";
...
let webpack_custom_config = {
    ...
    entry: {
        ...
        index: isDev ? (isLocalServer ? enrys.final_version.index : enrys.dev_version.index) : enrys.final_version.index,
    },
    ...
    output: {
        ...
        path: path.resolve(process.cwd(), compute_out_folder_name()), // складывать собранные фаилы в папку build
    },
...
function compute_out_folder_name() {
    return isDev ? "build/dev" : "build/final";
}
...
*/
    ...
    plugins.push(
        ...
        }) // удаляет все лишнее из папки build
    );
